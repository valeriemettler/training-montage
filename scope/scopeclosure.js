...
//Definition: 
// Closure is when a function can remember and access its lexical scope even when that function is invoked/executed outside its lexical scope.

//any of the various ways that functions can be passed around as values, and invoked in other locations outside of its author-time lexical scope, are all examples of observing/exercising closure
//Whenever and wherever you treat functions (which access their own respective lexical scopes) as values and pass them around, you are likely to see those functions exercising closure. Be that timers, event handlers, Ajax requests, cross-window messaging, web workers, or any of the other asynchronous (or synchronous!) tasks, and when you pass in a callback function, it's a closure.

//Loops + Closure
//When you put functions inside of loops, the output reflects the final value of the i after the loop terminates. All functions (each function that is run for each iteration) inside the loop share a reference to the same i (because they are closed over the same shared global scope which has only one  i in it).It's no different than if each of the functions inside the loop were just declared one right after the other, with no loop at all.
// What's missing? We need a new closured scope for each iteration of the loop.
//For pre-ES6, we can use an IIFE along with its own variable that holds a copy of the i value (example: var j = i;) at each iteration. (The IIFE creates a new scope for each iteration of the loop by declaring a function and immediately executing it. But an IIFE is just an empty do-nothing scope. It needs its own variable, with a copy of the i value at each iteration.)
//In ES6, let (which turns a block into a scope that we can close over) can be used to declare a variable and hold a copy of the value of i at each iteration. 

//Modules are a code pattern that leverages the power of closure because there are two "requirements" for the module pattern to be exercised:
//1. There must be an outer enclosing function, and it must be invoked at least once (each time creates a new module instance).
//2. The enclosing function must return back at least one inner function, so that this inner function has closure over the private scope, and can access and/or modify that private state.


//ES6 adds first-class syntax support for the concept of modules. When loaded via the module system, ES6 treats a file as a separate module. Each module can both import other modules or specific API members, as well export their own public API members. ES6 Module APIs are static (the APIs don't change at run-time), whereas pre-ES6 Function-based modules are not static and can be modified during run-time.
//ES6 modules do not have an "inline" format, they must be defined in separate files (one per module). The contents inside the module file are treated as if enclosed in a scope closure.


/////////

// Closures happen as a result of writing code that relies on lexical scope. They just happen. You do not even really have to intentionally create closures to take advantage of them. Closures are created and used for you all over your code.
...
function foo() {
	...
	function bar() {
		console.log( a ); // 2 ...the engine looks for the reference to a in this scope, but doesn't find it, so it goes up one scope and finds it with var a = 2;
	}
...
//Now I Can See
//The previous code snippets are somewhat academic and artificially constructed to illustrate using closure. But closure is something all around you in your existing code. Let us now see that truth.
...
// Deep down in the guts of the Engine, the built-in utility setTimeout(..) has reference to some parameter, probably called fn or func or something like that. Engine goes to invoke that function, which is invoking our inner timer function, and the lexical scope reference is still intact. Closure!
...
//While it is often said that IIFE (alone) is an example of observed closure, I would somewhat disagree, by our definition above.
var a = 2;
...
// But, that's not all! There's a special behavior defined for let declarations used in the head of a for-loop. This behavior says that the variable will be declared not just once for the loop, but each iteration. And, it will, helpfully, be initialized at each subsequent iteration with the value from the end of the previous iteration. Block scoping and closure working hand-in-hand.
for (let i=1; i<=5; i++) {
	setTimeout( function timer(){
		console.log( i );
	}, i*1000 );
...
//Modules
//Modules are a code pattern that leverages the power of closure. 
...
foo.identify(); // foo module...publicAPI is being returned, which is returning identify as identify1, which console logs the 'foo module' string being passed in as id
foo.change(); //this method changes identify to identify2 
foo.identify(); // FOO MODULE ...now that identify is identify2, the id is console logged in uppercase
